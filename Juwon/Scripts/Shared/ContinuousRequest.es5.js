        ...
        $.get('/StandardDocument/Search2?menuCode=' + menuCodeLv3 + '&languageCode=EN', function (response) {
            ...
            if (response.result.length > 0) {
                if (response.result[0].Content) {
                    var html = '';
                    html += '<h4>Name:  ' + response.result[0].Name + '</h4>';
                    html += '<h4>Full Direction:  ' + response.result[0].FullName + '</h4>';
                    //html += `<h5>Writer: ${response.result[0].Writer}</h5>`;
                    //html += `<h5>Description: ${response.result[0].Description}</h5>`;
                    html += '<div id="content"><h4>Content: </h4>' + response.result[0].Content + '</div>';
